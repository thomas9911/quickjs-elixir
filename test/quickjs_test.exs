defmodule QuickjsTest do
  ...
  describe "simple_run_script_timeout" do
    ...
    @tag skip: "crashes nif (stack overflow)"
    test "stack overflow function" do
      script = """
      function ack(m,n)
      {
          if (m == 0)
    ...
    test "large memory function" do
      script = """
      const arr = Array(1000000).fill(1)
      arr[arr.length - 1]
      """

      # maybe better error than Erlang exception?!!?
      {:error,
       %ErlangError{
         original:
           "Exception generated by quickjs:  out of memory\n    at <eval> (eval_script:1)\n"
       }} = Quickjs.simple_run_script_timeout(script, 500)
    end
  end
end
